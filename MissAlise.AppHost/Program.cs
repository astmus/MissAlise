...
var ps = builder.AddPostgres("pgsrv").WithLifetime(ContainerLifetime.Persistent);
...
builder.AddProject<Projects.MissAlise_PhotoService>("photo").WithHttpsEndpoint(port:7216).WithHttpEndpoint(7218).WithExternalHttpEndpoints().WithReference(db).WaitFor(db).WithReference(cache).WaitFor(cache);
builder.AddProject<Projects.MissAlise_VideoService>("video").WithHttpsEndpoint(port: 7220).WithHttpEndpoint(7222).WithExternalHttpEndpoints().WithReference(db).WaitFor(db).WithReference(cache).WaitFor(cache);
