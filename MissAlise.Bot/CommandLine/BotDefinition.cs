...
///// <summary>
///// Скомпилированное определение бота: RootCommand для парсинга + runtime-индекс для поиска leaf-команд,
///// проверки missing параметров и биндинга в модель.
///// </summary>
...
//    internal BotDefinition(
//        RootCommand root,
//        BotCommandDescription tree,
//        Dictionary<Command, LeafRuntime> leafByCommand,
...
//            if (optRes.Tokens.Count == 0 && opt.Arity.MinimumNumberOfValues > 0)
//                missing.Add(new MissingParam(p, "No values provided"));
//        }
...
//            throw new InvalidOperationException("Expected generic Option<T>");

//        var argType = optType.GetGenericArguments()[0];

//        // Try direct overload: GetValueForOption(Option<T>)
//        var mi = typeof(ParseResult)
//            .GetMethods(BindingFlags.Public | BindingFlags.Instance)
//            .Where(m => m.Name == "GetValueForOption" && m.GetParameters().Length == 1)
//            .FirstOrDefault(m => m.GetParameters()[0].ParameterType == optType);

//        if (mi is null)
//        {
//            var gen = typeof(ParseResult)
//                .GetMethods(BindingFlags.Public | BindingFlags.Instance)
//                .FirstOrDefault(m => m.Name == "GetValueForOption" && m.IsGenericMethodDefinition && m.GetParameters().Length == 1);

//            if (gen is null)
//                throw new InvalidOperationException("Cannot find ParseResult.GetValueForOption overload");

//            mi = gen.MakeGenericMethod(argType);
//        }
