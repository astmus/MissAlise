...
public interface IWorkflowUpdateResultVisitor
{
    Task VisitAsync(WorkflowHandleResult result, CancellationToken cancellationToken = default);
}

public abstract class WorkflowUpdateResultVisitor : IWorkflowUpdateResultVisitor
{
    private readonly IHandleContext _context;

    protected WorkflowUpdateResultVisitor(IHandleContext context)
    {
        _context = context;
    }
