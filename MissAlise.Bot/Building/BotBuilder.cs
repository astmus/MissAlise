...
public sealed class BotBuilder
{
    ...
    public BotDefinition Build()
    {
        ...
        foreach (var c in _root.SubCommands)
            rootCmd.AddCommand(BuildCommand(c, leafMap));

        return new BotDefinition(rootCmd, _root, leafMap);
    }

    private Command BuildCommand(BotCommandDescription d, Dictionary<Command, BotDefinition.LeafRuntime> leafMap)
    {
        ...
        foreach (var s in d.SubCommands)
            cmd.AddCommand(BuildCommand(s, leafMap));
