...
public sealed partial class BotBuilder : IBotBuilder
{
	...
	private static BotCommandDescription CreateCommandDescription(Type commandType, Command aliaseCmd, string path, out List<BotCommandDescription> subCommands)
	{
		...
		if (ctorArgs?.Any() == true)
		{
			...
			foreach (var arg in ctorArgs)
			{
				...
				var paramAttrs = arg.GetCustomAttributes<BotParameterAttribute>().ToList();
				var attr = BotParameterAttribute.Merge(paramAttrs);

				var valueMeta = attr?.CreateValueMeta(arg.ParameterType);
				...
				var opt = CreateOption(parameter, attr);
	...
	internal static Option CreateOption(BotParameterDescription arg, BotParameterAttribute? attr = null)
	{
